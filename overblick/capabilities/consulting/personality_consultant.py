...
class PersonalityConsultantCapability(CapabilityBase):
    ...
    async def consult(
        ...
        try:
            ...
            logger.error(
                "PersonalityConsultant: %s consultation failed: %s",
                name,
                e,
                exc_info=True,
            )
    ...
    def _get_system_prompt(self, name: str) -> str | None:
        ...
        prompt = build_system_prompt(
            personality,
            platform="Internal Consultation",
        )
    ...
    def _load_identity(self, name: str) -> Any:
        ...
        try:
            ...
            logger.info(
                "PersonalityConsultant: loaded personality '%s'",
                name,
            )
            return personality
        except FileNotFoundError:
            logger.warning(
                "PersonalityConsultant: personality '%s' not found",
                name,
            )
            return None
