...
def base_dir(tmp_path: Path) -> Path:
    """Create a temporary base directory with personality files."""
    # Copy personality files so the wizard can load them
    real_personalities = Path(__file__).parent.parent.parent / "overblick" / "personalities"
    test_personalities = tmp_path / "overblick" / "personalities"

    if real_personalities.exists():
        shutil.copytree(real_personalities, test_personalities)
