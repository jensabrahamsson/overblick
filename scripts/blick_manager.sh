#!/usr/bin/env bash
# Blick Manager — start/stop/restart/status per identity
# Usage: ./blick_manager.sh {start|stop|restart|status|logs} {anomal|cherry|all}
...
get_pid_file() {
    echo "$PID_DIR/$1/blick.pid"
}

get_log_file() {
    echo "$LOG_DIR/$1/blick.log"
}
...
start_identity() {
    ...
    nohup "$PYTHON" -m blick run "$identity" >> "$log_file" 2>&1 &
    local pid=$!
    echo "$pid" > "$pid_file"
